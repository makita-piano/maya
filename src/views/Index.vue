<template>
  <!DOCTYPE html>
  <html>
    <head>
      <meta charset="utf-8">
      <meta
        name="viewport"
        content="width=device-width, initial-scale=1"
      >

      <!--- common --->
      <!-- load -->
      <link
        rel="stylesheet"
        href="//cdnjs.cloudflare.com/ajax/libs/gridlex/2.3.1/gridlex.min.css"
      >
      <script
        src="//code.jquery.com/jquery-2.2.4.min.js"
        integrity="sha256-BbhdlvQf/xTY9gja0Dq3HiwQF8LaCRTXxZKRutelT44="
        crossorigin="anonymous"
      />
      <link
        href="https://fonts.googleapis.com/css?family=Noto+Sans+JP&display=swap"
        rel="stylesheet"
      >
      <link
        rel="stylesheet"
        href="common.css"
      >
      <script
        defer
        src="common.js"
      />

      <!-- meta -->
      <meta
        name="viewport"
        content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"
      >
      <meta charset="UTF-8">
      <meta
        property="og:type"
        content="website"
      >
      <meta
        name="msapplication-TileColor"
        content="#337AB7"
      >

      <!-- GA
    <script async src="//www.googletagmanager.com/gtag/js?id=xxx"></script>
    <script>
      window.dataLayer = window.dataLayer || [];
      function gtag(){dataLayer.push(arguments);}
      gtag('js', new Date());

      gtag('config', 'xxx');
    </script>
     -->

      <!--- unique --->
      <title>xxx</title>
      <meta
        property="og:url"
        content="xxx"
      >
      <meta
        property="og:site_name"
        content="xxx"
      >
      <meta
        property="og:description"
        content="xxx"
      >
      <meta
        name="description"
        content="xxx"
      >
    </head>

    <body id="body">
      <header />
      <div class="align-center margin-bottom-3">
        <img
          src="piano_inu.png"
          style="padding: 1rem;border: 1px solid #999;width: 60%"
        >
      </div>
      <div class="grid col-12 margin-bottom-2">
        <div class="grid-top-center col-6_sm-12 padding-0-2-pc padding-0-05-mobile">
          <div class="col-12 h3">
            Information
          </div>
          <ul class="information">
            <li>
              <div class="col-12">
                2019.07.14
              </div>
              <div class="grid col-12">
                <a href="#">
                  <div class="col-12">
                    xxxの動画が公開されました！
                  </div>
                </a>
              </div>
            </li>
            <li>
              <div class="col-12">
                2019.07.14
              </div>
              <div class="grid col-12">
                <a href="#">
                  <div class="col-12">
                    いろいろ<br>
                    いろいろ
                  </div>
                </a>
              </div>
            </li>
            <li>
              <div class="col-12">
                2019.07.14
              </div>
              <div class="grid col-12">
                <a href="#">
                  <div class="col-12">
                    xxxのチケットが発売されました！お求めはコチラから。
                    一般：3,000円
                    学生：2,000円
                  </div>
                </a>
              </div>
            </li>
          </ul>
          <div class="watch">
            <a href="information">
              一覧
            </a>
          </div>
        </div>
        <div class="grid-top col-6_sm-12 padding-0-2-pc padding-0-05-mobile">
          <div class="col-12 h3 margin-top-2-mobile">
            SNS Acount
          </div>
          <div class="col-12">
            <ul class="grid social">
              <li class="col-6_sm-12">
                <div class="twitter">
                  <a
                    href="https://twitter.com/aichan_nel"
                    target="_blank"
                    class="grid"
                  >
                    <div class="col-1 icon">
                      <img src="twitter_white.png">
                    </div>
                    <div class="col text">
                      <div class="service">
                        Twitter
                      </div>
                      <div class="name">
                        @xxx
                      </div>
                    </div>
                  </a>
                </div>
              </li>
              <li class="col-6_sm-12">
                <div class="youtube">
                  <a
                    href="https://www.youtube.com/channel/UC4YaOt1yT-ZeyB0OmxHgolA"
                    target="_blank"
                    class="grid"
                  >
                    <div class="col-1 icon">
                      <img src="youtube_white.png">
                    </div>
                    <div class="col text">
                      <div class="service">
                        YouTube
                      </div>
                      <div class="name">
                        Xxx Channel
                      </div>
                    </div>
                  </a>
                </div>
              </li>
            </ul>
          </div>
        </div>
      </div>
      <footer />
    </body>
  </html>
</template>
